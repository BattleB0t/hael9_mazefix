...
public class ArmourCommand extends CommandBase {
    ...
    public void processCommand(ICommandSender arg0, String[] arg1) throws CommandException {
        (new Thread(() -> {
            ...
            if (key.equals(""))
                player.addChatMessage((IChatComponent)new ChatComponentText(EnumChatFormatting.RED + "API key not set. Use /setkey."));
            if (arg1.length == 0) {
                username = player.getName();
                uuid = player.getUniqueID().toString().replaceAll("[\\-]", "");
                player.addChatMessage((IChatComponent)new ChatComponentText(DankersSkyblockMod.MAIN_COLOUR + "Checking armour of " + DankersSkyblockMod.SECONDARY_COLOUR + username));
            } else {
                username = arg1[0];
                player.addChatMessage((IChatComponent)new ChatComponentText(DankersSkyblockMod.MAIN_COLOUR + "Checking armour of " + DankersSkyblockMod.SECONDARY_COLOUR + username));
            ...
            try {
                ...
                player.addChatMessage((IChatComponent)new ChatComponentText(DankersSkyblockMod.ERROR_COLOUR + "An error has occurred while reading inventory data. See logs for more info."));
                ex.printStackTrace();
            }
        })).start();
    }
